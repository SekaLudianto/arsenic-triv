...
interface ModeSelectionScreenProps {
  ...
  onStartKnockout: (category: KnockoutCategory, useImportedOnly: boolean, maxPlayers: number) => void;
...
const classicCategories: { id: GameMode, name: string }[] = [
    ...
    { id: GameMode.TriviaBola, name: 'Trivia Bola' }, // NEW
    { id: GameMode.GuessTheCity, name: 'Tebak Kota' },
    { id: GameMode.ZonaBola, name: 'Zona Bola (Tebak Nama)' },
...
const validImportKeys = ['countries', 'trivia', 'cities', 'footballPlayers', 'footballClubs', 'footballStadiums', 'fruits', 'animals', 'kpopTrivia', 'movies', 'footballTrivia'];
...
const jsonExampleFormat = `{
  ...
  "footballTrivia": [
    { "question": "Siapa juara dunia?", "answer": "Jerman" }
  ],
  "fruits": [
    "Buah Iblis"
  ],
...
const ModeSelectionScreen: React.FC<ModeSelectionScreenProps> = ({ onStartClassic, onStartKnockout, onShowLeaderboard, onResetGlobalLeaderboard }) => {
  ...
  const [maxKnockoutPlayers, setMaxKnockoutPlayers] = useState<number>(32);
  ...
  const handleStartGame = () => {
    if (gameStyle === GameStyle.Classic) {
      ...
      onStartKnockout(knockoutCategory, useImportedOnly, maxKnockoutPlayers);
    }
  }
  ...
  const knockoutCategories: { id: KnockoutCategory, name: string }[] = [
    ...
    { id: 'TriviaBola', name: 'Trivia Bola' }, // NEW
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-xs mt-6">
          ...
          {gameStyle === GameStyle.Knockout && (
             <motion.div
              ...
              className="overflow-hidden"
            >
              <div className="p-3 bg-amber-50 dark:bg-gray-700/50 rounded-xl border border-amber-100 dark:border-gray-600 mb-3">
                  <div className="flex flex-col gap-1">
                      <label htmlFor="max-players" className="block text-xs text-left text-gray-500 dark:text-gray-400 font-semibold">Batasan Jumlah Pemain</label>
                      <input
                        type="number"
                        id="max-players"
                        value={maxKnockoutPlayers}
                        onChange={(e) => setMaxKnockoutPlayers(parseInt(e.target.value, 10) || 2)}
                        min="2"
                        className="w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-sky-200 dark:border-gray-600 rounded-lg text-slate-800 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-all dark:text-white"
                      />
                  </div>
