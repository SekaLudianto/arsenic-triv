...
import { Country, ChatMessage, LeaderboardEntry, RoundWinner, GameMode, AbcCategory, WordCategory, GameState, GameStyle, KnockoutPlayer, KnockoutBracket, KnockoutMatch, GameActionPayloads, KnockoutCategory, TriviaQuestion, GameAction, City, FootballStadium, LetterObject, FootballState } from '../types';
...
import { footballTrivia } from '../data/football_trivia';
...
export interface InternalGameState {
  ...
  maxPlayers: number; // New State for limit
...
const createInitialState = (): InternalGameState => ({
  ...
  footballState: {
      ...
      defenderId: null,
      ballPosition: 50,
      commentary: "Pertandingan akan segera dimulai...",
      actionState: 'idle',
  },
});
...
const gameReducer = (state: InternalGameState, action: GameAction): InternalGameState => {
  switch (action.type) {
    case 'START_GAME': {
      ...
      const { gameStyle, maxWinners, totalRounds, isHardMode, knockoutCategory, maxPlayers, classicRoundDeck, firstRoundData } = action.payload;
      ...
      if (gameStyle === GameStyle.Classic && firstRoundData) {
        ...
        return {
          ...
          maxWinners: maxWinners,
          knockoutCategory: knockoutCategory || null,
          maxPlayers: maxPlayers || 32, // Set player limit
    ...
    case 'PROCESS_COMMENT': {
        ...
        // --- KNOCKOUT FOOTBALL LOGIC: ONLY DEFENDER CAN ANSWER ---
        if (state.gameStyle === GameStyle.Knockout && state.footballState.defenderId) {
            if (message.userId !== state.footballState.defenderId) {
                // Ignore answers from non-defenders in this turn
        ...
        if (isCorrect) {
            ...
            } else { // Knockout mode SAVE/COUNTER action
                // In the new concept, if the defender answers correctly, they SAVE the ball and start a Counter Attack.
                // We do NOT award a point here. We just trigger the counter attack phase.
                
                // Swap roles for the counter attack
                const newAttackerId = state.footballState.defenderId!;
                const newDefenderId = state.footballState.attackerId!;

                return { 
                    ...state, 
                    isRoundActive: false, // Temporarily pause for animation
                    chatMessages: [ { ...message, isWinner: true }, ...state.chatMessages].slice(0,100),
                    footballState: {
                        ...state.footballState,
                        commentary: "PENYELAMATAN GEMILANG! Serangan balik cepat!",
                        actionState: 'save',
                    },
                    // We trigger a side-effect via a flag or immediately queuing next round state in useEffect
                };
            }
        }
        return { ...state, chatMessages: newChatMessages };
    }
    case 'TRIGGER_COUNTER_ATTACK': {
        return {
            ...state,
            footballState: {
                ...state.footballState,
                attackerId: action.payload.newAttackerId,
                defenderId: action.payload.newDefenderId,
                ballPosition: 50, // Reset to center then animate? Or animate from one side? Let's say 50 for simplicity now.
                actionState: 'dribble',
                commentary: "Serangan balik sedang dibangun..."
            }
        }
    }
    case 'TICK_TIMER': {
    ...
    case 'REGISTER_PLAYER':
        // Check player limit
        if (state.knockoutPlayers.length >= state.maxPlayers) return state;
    ...
    case 'START_MATCH': {
        return {
            ...
            currentWordCategory: null,
            currentStadium: null,
            footballState: { // Initialize football state
                attackerId: null, // Will be set in effect
                defenderId: null,
                ballPosition: 50, 
                commentary: "Kick-off babak ini!",
                actionState: 'idle'
            }
        };
    }
    case 'UPDATE_FOOTBALL_STATE': {
        return {
            ...state,
            footballState: { ...state.footballState, ...action.payload }
        }
    }
    ...
    case 'SET_KNOCKOUT_TRIVIA_BOLA': {
        return {
            ...state,
            gameMode: GameMode.TriviaBola,
            currentTriviaQuestion: action.payload.question,
            scrambledWord: scrambleWord(action.payload.question.answer, state.isHardMode),
            roundTimer: KNOCKOUT_ROUND_TIMER_SECONDS,
            isRoundActive: true,
    ...
    case 'SET_KNOCKOUT_KPOP_TRIVIA': {
        return {
            ...state,
            gameMode: GameMode.Trivia, // Use regular Trivia mode UI for now, or KpopTrivia if different
            currentTriviaQuestion: action.payload.question,
            scrambledWord: scrambleWord(action.payload.question.answer, state.isHardMode),
            roundTimer: KNOCKOUT_ROUND_TIMER_SECONDS,
            isRoundActive: true,
    ...
    case 'KNOCKOUT_QUESTION_TIMEOUT': {
        // If timeout in new concept: Defender failed to answer -> Attacker Scores
        const { knockoutMatchPoints, footballState, knockoutBracket, currentBracketRoundIndex, currentMatchIndex } = state;
        const currentMatch = knockoutBracket?.[currentBracketRoundIndex!]?.[currentMatchIndex!];
        
        let newPoints = { ...knockoutMatchPoints };
        let scorer = '';
        
        if (footballState.attackerId === currentMatch?.player1?.userId) {
            newPoints.player1++;
            scorer = currentMatch?.player1?.nickname || 'Player 1';
        } else {
            newPoints.player2++;
            scorer = currentMatch?.player2?.nickname || 'Player 2';
        }
        ...
        return { 
            ...
            footballState: {
                ...state.footballState,
                actionState: 'goal',
                commentary: `GOAAALLL! ${scorer} mencetak skor karena lawan gagal menjawab!`,
            }
        };
    }
...
export const useGameLogic = () => {
  ...
  const prepareNewDecks = useCallback((useImportedOnly: boolean = false) => {
    ...
    footballTriviaDeck.current = createDeck(footballTrivia, customQuestions.footballTrivia);
  ...
  const getNextFootballTrivia = useCallback(() => getNextUniqueItem(footballTriviaDeck, footballTrivia, 'footballTrivia', q => q.question), [getNextUniqueItem]);
  ...
  const startGame = useCallback((
    ...
    if (gameStyle === GameStyle.Classic) {
        ...
        } else if (firstRoundMode === GameMode.TriviaBola) {
            firstRoundData.triviaQuestion = getNextFootballTrivia();
        ...
        dispatch({ type: 'START_GAME', payload: { gameStyle, maxWinners, knockoutCategory: options?.knockoutCategory, maxPlayers: options?.maxPlayers } });
    }
  }, [prepareNewDecks, getNextCountry, getNextAbcCombo, getNextTrivia, getNextCity, getNextZonaBola, getNextFootballTrivia, getNextFruit, getNextAnimal, getNextKpopTrivia, getNextMovie]);
  ...
  const nextRound = useCallback(() => {
    ...
    } else if (nextGameMode === GameMode.TriviaBola) {
        payload.nextTriviaQuestion = getNextFootballTrivia();
  ...
  }, [state.round, state.totalRounds, state.gameStyle, state.classicRoundDeck, getNextCountry, getNextAbcCombo, getNextTrivia, getNextCity, getNextZonaBola, getNextFootballTrivia, getNextFruit, getNextAnimal, getNextKpopTrivia, getNextMovie]);
  ...
  // --- FOOTBALL SIMULATION LOGIC FOR KNOCKOUT ---
  useEffect(() => {
      if (state.gameState !== GameState.KnockoutPlaying) return;
      
      const currentMatch = getCurrentKnockoutMatch();
      if (!currentMatch || !currentMatch.player1 || !currentMatch.player2) return;

      // 1. Initial Start / Resuming a rally
      if (!state.isRoundActive && state.footballState.actionState !== 'goal') {
          // If we are idle or just saved, start a "Dribbling" phase that leads to an attack
          if (state.footballState.actionState === 'idle') {
              const attacker = Math.random() > 0.5 ? currentMatch.player1 : currentMatch.player2;
              const defender = attacker.userId === currentMatch.player1.userId ? currentMatch.player2 : currentMatch.player1;
              const targetGoalPos = attacker.userId === currentMatch.player1.userId ? 100 : 0; 

              dispatch({
                  type: 'UPDATE_FOOTBALL_STATE',
                  payload: {
                      attackerId: attacker.userId,
                      defenderId: defender.userId,
                      ballPosition: 50, // Start center
                      actionState: 'dribble',
                      commentary: `${attacker.nickname} menguasai bola di lini tengah...`
                  }
              });

              // Simulate approach to goal
              setTimeout(() => {
                  dispatch({
                      type: 'UPDATE_FOOTBALL_STATE',
                      payload: {
                          ballPosition: targetGoalPos === 100 ? 80 : 20, // Move closer
              ...
              setTimeout(() => {
                  // Determine next question type based on knockout category
                  dispatch({
                        type: 'UPDATE_FOOTBALL_STATE',
                        payload: {
                            ballPosition: targetGoalPos === 100 ? 95 : 5, // At the goal mouth
                            actionState: 'shooting',
                            commentary: "SHOOTING KERAS!! TEPIS SEKARANG!"
                        }
                  });
                  // Trigger the question logic
                  if(state.knockoutCategory === 'GuessTheCountry') {
                    ...
                    } else if (state.knockoutCategory === 'TriviaBola') {
                        const question = getNextFootballTrivia();
                        dispatch({ type: 'SET_KNOCKOUT_TRIVIA_BOLA', payload: { question } });
      ...
      // 2. Handle Save/Counter Attack Transition
      if (state.footballState.actionState === 'save' && !state.isRoundActive) {
          // The reducer sets 'save' state when correct answer is given. 
          // We wait a bit then trigger the counter attack sequence
          const timer = setTimeout(() => {
               // The player who was defending (and just answered correctly) becomes the attacker
               const previousDefenderId = state.footballState.defenderId;
               const newAttacker = previousDefenderId === currentMatch.player1.userId ? currentMatch.player1 : currentMatch.player2;
               const newDefender = newAttacker.userId === currentMatch.player1.userId ? currentMatch.player2 : currentMatch.player1;
               const targetGoalPos = newAttacker.userId === currentMatch.player1.userId ? 100 : 0;

               dispatch({
                  type: 'UPDATE_FOOTBALL_STATE',
                  payload: {
                      attackerId: newAttacker.userId,
                      defenderId: newDefender.userId,
                      ballPosition: 50, // Counter attack starts fast from mid/defensive half
                      actionState: 'dribble',
                      commentary: `Serangan balik cepat dari ${newAttacker.nickname}!`
                  }
               });
               
               // Fast forward to shoot
               setTimeout(() => {
                  dispatch({
                      type: 'UPDATE_FOOTBALL_STATE',
                      payload: {
                          ballPosition: targetGoalPos === 100 ? 95 : 5, 
                          actionState: 'shooting',
                          commentary: "PELUANG EMAS! TENDANGAN LANGSUNG!"
                      }
                  });
                  // Trigger question
                   if(state.knockoutCategory === 'GuessTheCountry') {
  ...
  }, [state.gameState, state.footballState.actionState, state.isRoundActive, getCurrentKnockoutMatch, getNextCountry, getNextTrivia, getNextFootballTrivia, getNextZonaBola, getNextFruit, getNextAnimal, getNextKpopTrivia, getNextMovie, state.knockoutCategory]);


  // This effect handles advancing the knockout game after a question is finished (GOAL case)
  useEffect(() => {
    // Only handle GOAL finish here. SAVE is handled in PROCESS_COMMENT/Football Loop
    if (state.gameState === GameState.KnockoutPlaying && !state.isRoundActive && state.footballState.actionState === 'goal') {
          ...
          // Reset for next kickoff
           dispatch({
              ...
              payload: {
                  actionState: 'idle',
                  ballPosition: 50,
                  commentary: "Kick-off ulang setelah gol."
              }
           });
        }
  ...
  }, [state.isRoundActive, state.gameState, state.knockoutMatchPoints, state.footballState.actionState, getCurrentKnockoutMatch]);
