...
// --- Knockout Mode Types ---
export type KnockoutCategory = 'GuessTheCountry' | 'Trivia' | 'ZonaBola' | 'TriviaBola' | 'GuessTheFruit' | 'GuessTheAnimal' | 'KpopTrivia' | 'ZonaFilm';
...
export interface FootballState {
    attackerId: string | null; // ID of the player currently attacking (has the ball)
    defenderId: string | null; // ID of the player currently defending (must answer)
    ballPosition: number; // 0 (Player 1 Goal) to 100 (Player 2 Goal). 50 is center.
    commentary: string; // Current match commentary text
    actionState: 'idle' | 'dribble' | 'shooting' | 'goal' | 'save';
}
...
export interface GameActionPayloads {
    'START_GAME': { 
      ...
      maxPlayers?: number; // New Payload for player limit
    ...
    'SET_KNOCKOUT_TRIVIA_BOLA': { question: TriviaQuestion }; // NEW
    ...
    'UPDATE_FOOTBALL_STATE': Partial<FootballState>;
    'TRIGGER_COUNTER_ATTACK': { newAttackerId: string, newDefenderId: string };
}
